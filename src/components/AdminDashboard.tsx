...
const AdminVideo: React.FC<AdminVideoProps> = ({ stream }) => {
  ...
  return (
    <div className="absolute bottom-4 right-4 w-32 aspect-video bg-black rounded-xl border border-white/20 overflow-hidden shadow-2xl z-10 scale-100 hover:scale-110 transition-transform cursor-pointer group">
      <video 
        ref={videoRef}
        autoPlay
        muted
      ...
      />
      <div className="absolute top-1 left-1 bg-black/40 px-1 rounded text-[8px] text-white font-bold uppercase">Admin (You)</div>
  ...
  const handleJoinInterview = async (sessionToJoin?: InterviewSession) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to get both video and audio, trying video only...", e);
        // Fallback to just video if audio fails (maybe no mic)
        stream = await navigator.mediaDevices.getUserMedia({ 
          video: { width: 640, height: 480 }
        });
      }
                    ...
                    <div className="relative aspect-video bg-neutral-900 rounded-2xl border-4 border-neutral-100 overflow-hidden shadow-inner group">
                      {(() => {
                        ...
                        const isDisconnected = sess.status === 'ONGOING' && 
                          sess.lastContact && 
                          (Date.now() - sess.lastContact > 15000);
                      ...
                      {/* Recruiter Overlay when joined */}
                      {isJoining && adminStream && (
                    ...
                    <div className="flex gap-2">
                      {!isJoining ? (
                        ...
                        <button 
                          ...
                          className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-700 transition-colors shadow-lg shadow-red-500/20"
                        >
                          <PhoneOff size={18} />
                          Leave Call
                        </button>
